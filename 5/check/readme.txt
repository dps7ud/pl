...
test2.cl: messy inheratice and method dispatch.
...
main.ml
  Everything else. Broken into four sections. They are:
    1. debugging/tracing
      - Since the overhead associated with these functions slowes the interpreter
      noticably, the code must be commented unless actively engaged in debugging.
    ...
    3. eval, of which, we will offer highlights.
        ...
        pass type checking so we need not worry about evaluating a let without prevously
        evaluating it's bindings.
        - Internally, self dispatch is represented by Dispatch(None, _, _)
        - New SELF_TYPE and self dispatch are simply routed to dispatch on so and 
        new type(so) respectively.
        - All arithmatic falls back on ocaml's Int32 arithmatic.
        - Equality falls back on ocaml's *==* operator.
        - Built in functions are implemented as a seperate expression kind 
